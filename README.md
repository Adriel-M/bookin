...
`src/bookin/graphql_client/` is generated by [ariadne-codegen](https://github.com/mirumee/ariadne-codegen) from `schema/hardcover.graphql` (vendored from [hardcover-docs](https://github.com/hardcoverapp/hardcover-docs)) and the operations in `queries/`. Don't edit it by hand — change a `.graphql` file and run `make codegen`. CI fails if the checked-in output doesn't match.

To pick up Hardcover schema changes, re-download `schema/hardcover.graphql` and regenerate; codegen fails loudly if an operation no longer matches the schema.
