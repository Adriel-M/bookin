...
`src/bookin/graphql_client/` is generated by [ariadne-codegen](https://github.com/mirumee/ariadne-codegen) from `schema/hardcover.graphql` (vendored from [hardcover-docs](https://github.com/hardcoverapp/hardcover-docs)) and the operations in `queries/`.
...
The schema is a snapshot, so it can lag Hardcover. If upstream renames a field we select, lookups start failing and each book logs `Hardcover lookup failed: ...` before falling back to its embedded metadata. Refresh `schema/hardcover.graphql` from [hardcover-docs](https://github.com/hardcoverapp/hardcover-docs) and re-run `make codegen` when that happens.
...
make codegen    # regenerate the Hardcover client from schema/ + queries/
```
