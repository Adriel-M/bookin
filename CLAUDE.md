...
├── graphql_client/   # GENERATED by ariadne-codegen — never hand-edit
...
- **The GraphQL client is generated, not hand-written.** `ariadne-codegen` builds `src/bookin/graphql_client/` from `schema/hardcover.graphql` plus `queries/*.graphql`; run `make codegen` after touching either, and CI fails if the checked-in output drifts. Queries deliberately use the *relational* columns (`contributions { author { name } }`, `book_series`, `image`) rather than Hardcover's `cached_*` `jsonb` columns, which would generate as `Any`. Every path is kept within Hardcover's max query depth of 3 — this is why the ISBN lookup filters `books` by their editions instead of querying `editions` directly.
...
- `schema/hardcover.graphql` is a vendored snapshot — schema changes are only noticed when it is re-downloaded and `make codegen` re-run
